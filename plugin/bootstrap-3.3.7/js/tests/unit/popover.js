$(function () {
  ...
  QUnit.test('should respect custom freemarker', function (assert) {
    assert.expect(3)
    var $popover = $('<a href="#">@fat</a>')
      .appendTo('#qunit-fixture')
      .bootstrapPopover({
  ...
  QUnit.test('should throw an error when freemarker contains multiple top-level elements', function (assert) {
    assert.expect(1)
    assert.throws(function () {
      $('<span data-toggle="popover" data-title="some title" data-content="some content">some text</span>')
        ...
        .bootstrapPopover('show')
    }, new Error('popover `freemarker` option must consist of exactly 1 top-level element!'))
  })
